...
一般的なキーボードは平面的にスイッチが配置されており、その配置を目的に応じて最適化することで、打鍵のしやすさを改善させたり、疲れにくくするなどといった、打鍵感の向上への取り組みがおこなわれています。  
また、行ごとに高さの異なる形状（※）のキーキャップを使用することで、ホームポジションから遠いキーへ指が届きやすくする最適化がおこなわれています。  

しかし、スイッチの配置が平面上では縦と横の2方向しか最適化できないため、その最適化には限界があります。  
また、市販されているキーキャップの形状は行単位で変化するため、指の長さや、ホームポジションからの離れ具合に応じた最適化はおこなわれていません。  

〓つまり、平面的な最適化だけでは、なかなかキビシイ。
...
〓3D的なアプローチは究極的な最適化がおこなえるが、それゆえに、一般的なキーボードから乗り換えるのが厳しくなっている。
