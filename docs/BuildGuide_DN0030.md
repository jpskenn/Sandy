...
| Base用スイッチプレート | 1 | 捨て基板（捨て板）を切り離して使用 |
| Top用スイッチプレート | 1 ||
| ボトムプレート | 1 ||
...
| ワッシャー<br>M2 | 19 | 内径2.3mm 外径4.3mm 厚さ0.4mm<br>[ウィルコ FW-0204-04N](https://wilco.jp/products/F/FW-N.html#page3) |
...
| スイッチ用ソケット（MX） | 68 | 予備1個含む<br>[MXスイッチ用 Kailh PCBソケット](https://talpkeyboard.net/items/5e02c5405b120c792616bcf9) |
...
※出荷時期によって、基板の仕様や部品の品番などを機能的に同等なものへ変更することがあります。
...
1. （該当者のみ）LED経路を`Front`にジャンパする
    ...
    *※出荷時期によっては、出荷時にジャンパされていることがあります。*
...
1. （オプション）インジケータLEDを取り付ける
    ...
    基板に描かれた印にLEDの端子の切り欠き部分を合わせ、270度以下の小手先温度ではんだ付けします。
