...
- 組み立て難易度
  ...
  - 通常版以外のキット 難易度： **★★★★☆ そこそこ難しい**
    ...
    また、パーツが小さくはんだブリッジを目視確認するのが困難なため、テスターでの確認がほぼ必須となります。  
    はんだ付けを数回やった程度の方には、かなりチャレンジングな作業になるでしょう。  

    コネクタの取り付けに失敗しても、致命傷ではなく、修正すれば普通に復旧できるので、逆に安心とも言えます。
...
なお、`（オプション）`の手順は、こだわりたい方に向けた内容であり、不要であれば作業をとばしてもかまいせん。

また、`（該当者のみ）`の手順は、条件に該当する場合のみ作業します。

基板うら側には`Back Side`の表記があります。  
おもて側とうら側を間違えないように、確認しながら作業を進めます。
...
1. プレート上部の捨て基板（プレート外側の枠）をミシン目に沿って切り離す
    ...
    固くて折り取るのが難しい場合は、ミシン目をうら側からカッターで10-20回ほどなぞって切れ込みを入れると折りやすくなります。

    切り離した箇所が尖っている場合は、やすりなどで滑らかにします。

    ![Base用プレート切り離し箇所](/assets/BuildGuide_DN0030/plate_base_cut.png)
