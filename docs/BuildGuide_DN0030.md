...
初心者にはちょっと難しいかもですが、まぁなんとかなるでしょう。

特別版キットの組み立て難易度は **そこそこ難しいレベル** です。  
慣れるまではSHコネクタの取り付けに失敗しがちで、テスターでの確認がほぼ必須となります。
...
- 基板のおもて側とうら側をよく確認する
  ...
  基板のおもて側とうら側を間違えないよう、基板うら側の`Back Side`表記をよく確認してください。  
...
| ロータリーエンコーダ用ノブ（つまみ） | 必要数 | ロータリーエンコーダを使用する場合 |
| LED<br>SK6812MINI-E | 16 | アンダーグローLEDを取り付ける場合<br>[マイコン内蔵ＲＧＢＬＥＤ　ＳＫ６８１２ＭＩＮＩ－Ｅ](https://akizukidenshi.com/catalog/g/gI-15478/) |
| GH60互換ケース | 1 | ボトムプレートを使わず、ケースへ取り付ける場合。<br><br>ケースへの基板の固定には中央と中央下のネジを使用せず、左右2本ずつ合計4本のネジだけで取り付けます。<br>ケース内部の構造によっては取り付けに加工が必要になったり、取り付けできないことがるかもしれません。<br><br>取り付け確認済みのケースは以下の通りです。<br>- KBDfans TOFU 60% ALUMINUM CASE<br>- KBDfans 5° FIVE DEGREE 60% ALUMINUM CASE<br>- 60% プラスチックケース（簡単な加工が必要になる場合があります） |
...
1. コネクタへケーブルを差し込む（3本）
    ...
    コネクタが基板から外れないように、コネクタを上から押さえたり後ろを支えたりしながらケーブルを差し込むと安心です。
...
1. うら側から、スイッチソケットを取り付ける（34〜35個）
    ...
    左ControlとEnterは、使用するキーのサイズに合わせてソケットを取り付けます。  
    ロータリーエンコーダを取り付ける場合は、その場所にソケットを取り付ける必要はありません。

1. （オプション）うら側から、インジケータLEDを取り付ける（2個）
...
1. （オプション）おもて側から、アンダーグローLEDを取り付ける（16個）
...
1. （該当者のみ）おもて側から、SHコネクタを取り付ける（6個）
    ...
    隣同士の端子がはんだで繋がっていないか確認します。

    可能であれば、コネクタのうら側にあるスルーホールにテスターのプローブを当て、隣同士で同通しないことを確認します。

1. （オプション）おもて側から、ロータリーエンコーダを取り付ける
...
1. 捨て基板（プレート外側の枠）をミシン目に沿って切り離す
    ...
    固くて折り取るのが難しい場合は、ミシン目をうら側からカッターで10-20回ほどなぞると折りやすくなります。
...
スイッチの端子が曲がっていないことを確認してから、まっすぐ押し込んで取り付けます。

1. 基板Midのスイッチを、基板TopのMid用スイッチプレートを介して取り付ける

1. 基板Baseのスイッチを、Base用スイッチプレートを介して取り付ける

  💡ヒント：最初にプレート4隅と中央付近のスイッチを取り付けておいてから、基板Baseに乗せる方法もあります。
