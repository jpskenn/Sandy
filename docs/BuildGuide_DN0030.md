...
- 組み立て難易度
  ...
  - 通常版キット 難易度： **★★★☆☆ ふつう**
    ...
    初めてキーボードキットを組み立てる方や不慣れな方にはちょっと難しいかもですが、まぁなんとかなるでしょう。

  - 通常版以外のキット 難易度： **★★★★☆ そこそこ難しい**

    コツをつかむまでは、SHコネクタの取り付けに失敗しがちです。  
    パーツが小さく、はんだブリッジを目視確認するのが困難なため、テスターでの確認がほぼ必須となります。  
    はんだブリッジ時の修正などを考えると、はんだ付けを数回やった程度の方には、かなりチャレンジングな作業になるでしょう。
...
| 6 | Top用スイッチプレート | 1 ||
| 7 | ボトムプレート | 1 ||
| 8 | JST SH 4pin<br>（Qwiicケーブル） | 6 | [PRT-17260](https://www.digikey.jp/ja/products/detail/sparkfun-electronics/PRT-17260/13629028) |
| 9 | スペーサー<br>両メネジ M2 3mm | 16 | 丸型、六角いずれも使用可能<br>[ヒロスギ ARB-2003E](https://hirosugi.co.jp/products/B/ARB-E.html#bx3) |
...
| GH60互換ケース | 1 | ボトムプレートを使わず、ケースへ取り付ける場合。<br><br>ケースへの基板の固定には、中央と中央下のネジは使用しません。左右2本ずつ、合計4本のネジだけで取り付けます。<br>すべてのGH60型ケースへの対応を確認していないため、ケース内部の構造によっては取り付けに加工が必要になったり、取り付けできないことも考えられます。<br><br>取り付け確認済みのケースは以下の通りです。<br>- KBDfans TOFU 60% ALUMINUM CASE<br>- KBDfans 5° FIVE DEGREE 60% ALUMINUM CASE<br>- 60% プラスチックケース（簡単な加工が必要になる場合があります） |
...
- あると便利な工具
  ...
  - マジックペン
    ...
    [プロッキー](https://www.mpuni.co.jp/products/felt_tip_pens/water_based/sign_pen/prockey/pm_150tr.html)や[ポスカ](https://www.mpuni.co.jp/products/felt_tip_pens/water_based/posca/standard.html)など。  
  ...
  - テスター
    ...
    **通常版以外のキットでSHコネクタを取り付ける場合は、ほぼ必須と言えます。**

  - フラックスやハンダ吸い取り線

    はんだ付けの修正や、部品の取り外しなどがしやすくなります。

参考商品：[工具セット](https://shop.yushakobo.jp/products/a9900to)

## 組み立て手順

以降の手順に沿って組み立てます。

なお、`（オプション）`の手順は、こだわりたい方に向けた手順であり、不要であれば作業をとばしてもかまいせん。

また、`（該当者のみ）`の手順は、条件に該当する場合のみおこないます。

基板うら側には`Back Side`の表記があります。  
基板のおもて側とうら側を確認しながら、作業を進めます。
...
1. 方向を合わせて、コネクタへケーブルを差し込む（3本）
    ...
    **ケーブルの端子側が上**になるように、方向を合わせて差し込みます。
    ...
    コネクタが基板からもげてしまわないように、コネクタを上から押さえたり後ろを支えたりしながらケーブルを差し込むと安心です。
...
1. （オプション）うら側から、インジケータLEDを取り付ける（2個）
    ...
    基板に描かれた印にLEDの端子の切り欠き部分を合わせてはめこみ、270度以下の温度ではんだ付けします。
...
1. 基板Baseのスイッチを、Base用スイッチプレートを介して取り付ける
    ...
    💡ヒント：最初にプレート4隅と中央付近のスイッチを取り付けておいてから、基板Baseに乗せると簡単です。
