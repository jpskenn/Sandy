...
| キーキャップ<br>Cherry MX（互換）スイッチ用 | 必要数 | 一般的な英語104のキーキャップセットに、スペースキー用として1.5uのキー2個を追加すると全て埋めることができます。<br>1.5uの2個は文字の入っていないブランクのDSAなどを使用すると良いでしょう。<br>参考： [Sandy（DN0020）のレイアウト](http://www.keyboard-layout-editor.com/#/gists/19f42cd466b4f3b269f38e3bdd6bf532) |
| GH60型, Poker型ケース<br>または<br>専用ボトムプレートキット | 1 | Jones, Nora用も使用可能 |
| USBケーブル （Type-C） | 1 | |
...
発送前に基板Baseのすべてのパッドが正常に反応することを確認済みですが、輸送時の破損なども考えられるため、念のため確認を行います。
...
1. キースイッチの反応を確認する
    ...
    基板Baseのスイッチソケット取り付け用パッド（銀色の四角い端子部分）をピンセットなどでショートさせ、画面上のキーの色が青に変わることを確認します。  
...
1. 基板側面をマジックで塗る
    ...
    奥側のTop用基板部分はケース壁面に隠れるため、塗らなくてもよいでしょう。

    ロープロファイルのケースやボトムプレートと組み合わせる場合は、すべての側面を塗ります。
...
1. 基板側面をマジックで塗る
    ...
    ロープロファイルのケースやボトムプレートと組み合わせる場合は、外から見えそうなところをすべて塗ります。
